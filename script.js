...
function initAgenticDashboard() {
    ...
    function drawChart() {
        ...
        const pts1 = liveData.revenue, pts2 = liveData.ops;
        const pad = {l:8,r:8,t:10,b:20};
        const chartW = w-pad.l-pad.r, chartH = h-pad.t-pad.b, maxV = 110;
        ...
        for(let i=0;i<=4;i++){const y=pad.t+(i/4)*chartH;ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();}

        // Ops area
        ctx.beginPath(); ctx.moveTo(xPos(0),yPos(pts2[0]+waveOffset));
        for(let i=1;i<pts2.length;i++){const cx=xPos(i-0.5);ctx.bezierCurveTo(cx,yPos(pts2[i-1]+waveOffset),cx,yPos(pts2[i]+waveOffset),xPos(i),yPos(pts2[i]+waveOffset));}
        ctx.lineTo(xPos(pts2.length-1),h);ctx.lineTo(xPos(0),h);ctx.closePath();
        const g2=ctx.createLinearGradient(0,0,0,h);g2.addColorStop(0,'rgba(239,101,38,0.18)');g2.addColorStop(1,'rgba(239,101,38,0)');
        ...
        // Revenue area
        ctx.beginPath();ctx.moveTo(xPos(0),yPos(pts1[0]-waveOffset));
        for(let i=1;i<pts1.length;i++){const cx=xPos(i-0.5);ctx.bezierCurveTo(cx,yPos(pts1[i-1]-waveOffset),cx,yPos(pts1[i]-waveOffset),xPos(i),yPos(pts1[i]-waveOffset));}
        ctx.lineTo(xPos(pts1.length-1),h);ctx.lineTo(xPos(0),h);ctx.closePath();
        const g1=ctx.createLinearGradient(0,0,0,h);g1.addColorStop(0,'rgba(69,132,237,0.25)');g1.addColorStop(1,'rgba(69,132,237,0)');
        ...
        const dotX=xPos(pts1.length-1),dotY=yPos(pts1[pts1.length-1]-waveOffset);
        ctx.beginPath();ctx.arc(dotX,dotY,4+dp*3,0,Math.PI*2);ctx.fillStyle=`rgba(69,132,237,${0.15+dp*0.15})`;ctx.fill();
        ctx.beginPath();ctx.arc(dotX,dotY,4,0,Math.PI*2);ctx.fillStyle='#4584ed';ctx.fill();
        ctx.beginPath();ctx.arc(dotX,dotY,2,0,Math.PI*2);ctx.fillStyle='#fff';ctx.fill();

        ctx.fillStyle='rgba(255,255,255,0.35)';ctx.font='9px Inter,sans-serif';ctx.textAlign='center';
    ...
    function fluctuateMetrics() {
        ...
        if(r){const v=17+Math.floor(Math.random()*3);r.textContent=`+${v}%`;}
        const s=document.getElementById('dash-sources');
        if(s){s.textContent=['12 live','13 live','12 live'][Math.floor(Math.random()*3)];}
    }

    function animate(){tick++;drawChart();requestAnimationFrame(animate);}
    animate();
